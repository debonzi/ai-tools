...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "db11-crew"
LOADER = importlib.machinery.SourceFileLoader("db11_crew", str(SCRIPT))
...
class CrewTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.state_root = self.home / ".local" / "state" / "db11-crew"
        ...
        self.environment = mock.patch.dict(
            os.environ,
            {
                "DB11_CREW_STATE_DIR": str(Path(self.temporary.name) / "ignored-state"),
                "DBZ_CREW_STATE_DIR": str(Path(self.temporary.name) / "ignored-legacy-state"),
    ...
    def create_repository(self, name: str = "repo") -> Path:
        ...
        self.git(repository, "config", "user.name", "DB11 Crew Test")
        self.git(repository, "config", "user.email", "db11-crew@example.invalid")
        return repository

    def test_state_root_is_literal_and_leaves_legacy_state_untouched(self) -> None:
        legacy_root = self.home / ".local" / "state" / "dbz-crew"
        (self.home / ".local").mkdir(mode=0o700)
        legacy_root.parent.mkdir(mode=0o700)
        legacy_root.mkdir(mode=0o755)
        os.chmod(legacy_root, 0o755)
        marker = legacy_root / "legacy.json"
        ...
        self.assertNotEqual(CREW.state_root(), Path(os.environ["DB11_CREW_STATE_DIR"]))
        ...
        self.assertEqual(stat.S_IMODE(legacy_root.stat().st_mode), 0o755)
    ...
    def test_monitor_launch_does_not_propagate_state_environment(self) -> None:
        ...
        self.assertFalse(any("DB11_CREW_STATE_DIR" in argument for argument in arguments))
    ...
    def test_read_only_resource_cleanup_accepts_recorded_dirty_baseline(self) -> None:
        ...
        self.git(repository, "add", "tracked.txt")
        self.git(repository, "commit", "-m", "test: initial")
        branch = "db11-crew/test/readonly"
    ...
    def test_pi_monitor_writes_session_event_without_prompting_the_principal(self) -> None:
        ...
        def fake_command(args: list[str], cwd: Path | None = None):
            ...
            if args[:4] == ["herdr", "agent", "read", "worker-agent"]:
                return completed(stdout="DB11-CREW RESULT: done\n")
    ...
    def test_isolated_read_only_monitor_fails_and_retains_changed_snapshot(self) -> None:
        ...
        def fake_command(args: list[str], cwd: Path | None = None):
            if args[:4] == ["herdr", "agent", "prompt", "worker-agent"]:
                return completed(stdout='{"agent_status":"idle"}\n')
            if args[:4] == ["herdr", "agent", "read", "worker-agent"]:
                return completed(stdout="DB11-CREW RESULT: inspected\n")
            if args[:3] == ["herdr", "notification", "show"]:
                return completed()
            raise AssertionError(f"unexpected command: {args}")
        ...
        self.assertIn("DB11-CREW READ-ONLY VIOLATION", output)
    ...
    def test_in_place_read_only_monitor_warns_without_failing_and_finalizes(self) -> None:
        ...
        output = CREW.result_path(pane, "worker-one").read_text(encoding="utf-8")
        self.assertIn("DB11-CREW READ-ONLY WARNING", output)
    ...
    def test_completion_updates_are_locked_and_event_files_are_private(self) -> None:
        ...
        CREW.write_private_text(result_file, "DB11-CREW RESULT: done\n")
