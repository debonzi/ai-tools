...
const EVENT_TYPE = "db11-crew-event";
const DELIVERED_ENTRY_TYPE = "db11-crew-event-delivered";
const DEFAULT_POLL_MS = 250;
const PRIVATE_DIRECTORY_MODE = 0o700;
const PRIVATE_FILE_MODE = 0o600;
const STATE_DIRECTORY_PARTS = [".local", "state", "db11-crew"] as const;
...
export default function db11CrewEventsExtension(pi: ExtensionAPI, options: ExtensionOptions = {}) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			...
			const pollMs = parsePollInterval(process.env.DB11_CREW_EVENT_POLL_MS);
			...
			runtime = undefined;
			ctx.ui.notify(`DB11 Crew event delivery is disabled: ${errorMessage(error)}`, "warning");
		}
	});
	...
	async function processPending(current: ActiveRuntime): Promise<void> {
		...
		try {
			...
			for (const name of entries.filter((entry) => entry.endsWith(".json")).sort()) {
				...
				try {
					...
					if (!event || !(await isSafeResultFile(event.result, current.location))) {
						...
						current.ctx.ui.notify("DB11 Crew ignored an invalid completion event.", "warning");
						continue;
					}
			...
			if (!current.stopped) {
				current.ctx.ui.notify(`DB11 Crew event delivery failed: ${errorMessage(error)}`, "warning");
			}
		} finally {
...
function completionMessage(event: CrewCompletionEvent): string {
	...
	return [
		`DB11-CREW EVENT: worker ${event.task_id} ${event.phase} is ${event.status}.`,
		`Read ${event.result}, load the db11-crew skill, inspect the worker state with its bundled status command,`,
...
function currentUid(): number {
	...
	if (uid === undefined) throw new Error("Secure DB11 Crew state handling requires a POSIX user ID.");
	return uid;
}
...
function secureOpenFlags({
	...
	for (const name of ["O_NOFOLLOW", ...(directory ? ["O_DIRECTORY"] : [])]) {
		if (typeof fsConstants[name as keyof typeof fsConstants] !== "number") {
			throw new Error(`Secure DB11 Crew state handling is unavailable: missing ${name}.`);
		}
	}
...
function assertOwnedDirectory(stats: Stats, path: string, privateDirectory: boolean): void {
	if (!stats.isDirectory()) throw new Error(`DB11 Crew state path is not a directory: ${path}`);
	if (stats.uid !== currentUid()) {
		throw new Error(`DB11 Crew state directory is not owned by the current user: ${path}`);
	}
	if (!privateDirectory && (stats.mode & 0o022) !== 0) {
		throw new Error(`DB11 Crew state parent is writable by other users: ${path}`);
	}
}

function assertOwnedFile(stats: Stats, path: string): void {
	if (!stats.isFile()) throw new Error(`DB11 Crew state path is not a regular file: ${path}`);
	if (stats.uid !== currentUid()) {
		throw new Error(`DB11 Crew state file is not owned by the current user: ${path}`);
	}
}
...
async function rejectRepositoryLocalStateRoot(root: string): Promise<void> {
	...
	while (true) {
		if (await inspectPath(resolve(current, ".git"))) {
			throw new Error(`DB11 Crew state directory cannot be inside a Git worktree: ${root}`);
		}
...
async function ensureDirectoryComponent(path: string, create: boolean, privateDirectory: boolean): Promise<void> {
	...
	if (!info) throw Object.assign(new Error(`DB11 Crew state directory is unavailable: ${path}`), { code: "ENOENT" });
	if (info.isSymbolicLink()) throw new Error(`DB11 Crew state directory cannot be a symbolic link: ${path}`);
...
function stateRelative(path: string, location: StateLocation): string {
	...
	if (normalized !== path) throw new Error(`DB11 Crew state path is not normalized: ${path}`);
	const result = relative(location.root, normalized);
	if (result.startsWith("..") || isAbsolute(result)) {
		throw new Error(`State path is outside the DB11 Crew state directory: ${path}`);
	}
	return result;
}
...
async function ensureStateRoot(location: StateLocation, create: boolean): Promise<void> {
	if (resolveStateRoot(location.homeDirectory) !== location.root) {
		throw new Error(`Unexpected DB11 Crew state root: ${location.root}`);
	}
...
async function ensureStateDirectory(
	...
	for (const part of statePath.split("/")) {
		if (!part || part === "." || part === "..") {
			throw new Error(`DB11 Crew state path contains an unsafe component: ${directory}`);
		}
		current = resolve(current, part);
...
async function openPrivateFile(path: string, location: StateLocation): Promise<FileHandle> {
	...
	if (!before) throw Object.assign(new Error(`DB11 Crew state file is unavailable: ${path}`), { code: "ENOENT" });
	if (before.isSymbolicLink()) throw new Error(`DB11 Crew state file cannot be a symbolic link: ${path}`);
	...
	try {
		...
		if (before.dev !== after.dev || before.ino !== after.ino) {
			throw new Error(`DB11 Crew state file changed while it was opened: ${path}`);
		}
...
async function validateReplaceableFile(path: string, location: StateLocation): Promise<void> {
	...
	if (!info) return;
	if (info.isSymbolicLink()) throw new Error(`DB11 Crew state file cannot be a symbolic link: ${path}`);
...
async function renamePrivateFile(source: string, destination: string, location: StateLocation): Promise<void> {
	if (dirname(source) !== dirname(destination)) {
		throw new Error("DB11 Crew state files can only be renamed within one directory.");
	}
	const sourceHandle = await openPrivateFile(source, location);
	await sourceHandle.close();
	if (await inspectPath(destination)) {
		throw new Error(`Refusing to replace an existing DB11 Crew state path: ${destination}`);
	}
