...
import db11CrewEventsExtension, {
...
function makeHarness(sessionId: string, initialEntries: FakeEntry[] = [], homeDirectory?: string) {
	...
	db11CrewEventsExtension(pi as any, { homeDirectory });
...
async function writeEvent(
	...
	await writeFile(result, "DB11-CREW RESULT: done\n");
...
async function withRuntime(
	...
	const homeDirectory = await mkdtemp(resolve(tmpdir(), "db11-crew-events-test-"));
	...
	const previous = {
		state: process.env.DB11_CREW_STATE_DIR,
		legacyState: process.env.DBZ_CREW_STATE_DIR,
		xdg: process.env.XDG_STATE_HOME,
		herdr: process.env.HERDR_ENV,
		socket: process.env.HERDR_SOCKET_PATH,
		pane: process.env.HERDR_PANE_ID,
		poll: process.env.DB11_CREW_EVENT_POLL_MS,
	};
	process.env.DB11_CREW_STATE_DIR = resolve(homeDirectory, "ignored-state");
	process.env.DBZ_CREW_STATE_DIR = resolve(homeDirectory, "ignored-legacy-state");
...
test("uses the literal account state path without environment overrides", () => {
	const previousState = process.env.DB11_CREW_STATE_DIR;
	const previousXdg = process.env.XDG_STATE_HOME;
	process.env.DB11_CREW_STATE_DIR = "/tmp/ignored-state";
	process.env.XDG_STATE_HOME = "/tmp/ignored-xdg";
	try {
		assert.equal(resolveStateRoot("/home/example"), "/home/example/.local/state/db11-crew");
		assert.equal(resolveStateRoot(), resolve(userInfo().homedir, ".local", "state", "db11-crew"));
	} finally {
		if (previousState === undefined) delete process.env.DB11_CREW_STATE_DIR;
		else process.env.DB11_CREW_STATE_DIR = previousState;
...
test("validates result paths inside the private state root", () => {
	...
	assert.ok(
		validateCompletionEvent(
			{ ...base, result: "/tmp/db11-state/results/pane/result.md" },
			"session-1",
			root,
		),
	);
...
test("recovers a pending event and always delivers it as a follow-up", { concurrency: false }, async () => {
	await withRuntime(async (stateRoot, homeDirectory) => {
		...
		await writeEvent(
			...
			sessionId,
			"event-recovery",
			"DB11-CREW EVENT: read-only worker worker-one implementation is done.",
		);
...
test("removes an already delivered pending event without redelivery", { concurrency: false }, async () => {
	await withRuntime(async (stateRoot, homeDirectory) => {
		...
		const harness = makeHarness(sessionId, [
			{ type: "custom", customType: "db11-crew-event-delivered", data: { id: "event-idempotent" } },
		], homeDirectory);
		await harness.handlers.get("session_start")?.({}, harness.ctx);
		await waitFor(async () => {
			try {
