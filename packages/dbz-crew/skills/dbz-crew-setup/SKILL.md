---
name: dbz-crew-setup
description: Validate DBZ Crew prerequisites and, after a complete plan and explicit confirmation, install the official Herdr Pi integration when needed. Use only when the user explicitly invokes this setup skill or directly requests DBZ Crew setup.
...
Run this workflow only after an explicit user invocation or request for DBZ Crew setup. Do not install unrelated software, edit Pi package filters, change Pi trust decisions, or mutate any state before the user approves the complete plan.

DBZ Crew's skill and event extension are bundled in one Pi package and load together. This workflow never edits package resource filters.
...
If the integration is already current, do not propose or run a mutation. Otherwise ask for explicit confirmation of the complete plan. A general request to set up DBZ Crew is not confirmation to run the installation command; obtain a separate clear approval after presenting the plan.
