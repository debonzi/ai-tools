...
SPEC = importlib.util.spec_from_file_location("db11_issues_cli", SCRIPT)
...
class Db11IssuesTest(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="db11-issues-test-")
    ...
    def test_concurrent_creation_reserves_unique_numbers(self) -> None:
        ...
        self.assertEqual(list((self.root / "open").glob(".*.db11-issues-reservation")), [])
